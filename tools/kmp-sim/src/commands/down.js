...
export async function down(cwd, flags) {
  ...
  if (flags.all) {
    ...
    if (runs.length === 0) {
      info('no kmp-sim runs in flight');
      clearSession(cwd);
      return;
    }
  ...
  if (!session) {
    info(`no kmp-sim session recorded ${dim('(try: kmp-sim down --all)')}`);
    return;
  }
  ...
  if (others.length) {
    warn(`${others.length} other kmp-sim run(s) still in flight — ${dim('kmp-sim down --all')}`);
  }
